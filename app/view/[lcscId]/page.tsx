...
export default function ViewPage() {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      // Add footprint file
      if (footprint) {
        const kicadFootprint = convertToKiCadFootprint(footprint, footprint.originX, footprint.originY, lcscId);
  ...
  const handleDownload = async (type: 'symbol' | 'footprint' | 'step' | 'obj') => {
    ...
    try {
      ...
      if (type === 'symbol' && schematic) {
        ...
        const kicadContent = convertToKiCadFootprint(footprint, footprint.originX, footprint.originY, lcscId);
        const blob = new Blob([kicadContent], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
